	...
	Allspeak = function(args) {
		...
		var getJsonData = function(url, callback, async) {
			...
			$.ajax({
				...
				console.warn('An error occurred while loading the translation data: "' + textStatus + '"');
				console.warn(jqxhr);
				return false;
			});
		};
		...
		var updateElement = function(el, trans) {
			if (this.debug) {
				console.info('Updating element:', el.get(0));
			};
		...
		var updateAll = function(_this, args) {
			var _els = args.scope.find('*');
			_els.each(function(index, el) {
				var _key = $(el).attr(_this.keyAttrName);
